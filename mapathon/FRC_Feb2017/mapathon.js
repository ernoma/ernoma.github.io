...
$(document).ready(function () {
	...
	var centerLng = data.centerLng;

	var areaPoly = L.polygon(data.latLngs, { color: '#54cc54', fill: false });
	areaPoly.addTo(map);
...
function calculateBuildingStatistics(elements) {
    ...
    var modifiedPercentage = modifiedCount / elements.length * 100;
...
function readCoords(filePath, callback) {
	...
	var data = geojsonData.features[0].geometry.coordinates[0];

	var latSum = 0;
	var lngSum = 0;
	
	for (var i = 0; i < data.length - 1; i++) {
	    latSum += Number(data[i][1]);
	    lngSum += Number(data[i][0]);
	    coordData.latLngs.push(L.latLng(data[i][1], data[i][0]));
	    //polyString += data[i][1].toFixed(9) + " " + data[i][0].toFixed(9) + " ";
	}
	//console.log(polyString);

	coordData.centerLat = latSum / data.length;
	coordData.centerLng = lngSum / data.length;
