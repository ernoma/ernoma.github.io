...
def createPoly(poly_file):
    with open(poly_file, 'r') as data_file:
...
parser.add_argument("poly_file", help="file that contains coordinates for the area that changes are looked up from. The file can be created with the get_poly.py")
parser.add_argument("date", help="date of the changes that are looked up in format year-mm-dd, e.g. 2017-02-08")
parser.add_argument("min_hour_utz", help="changes are looked up from this hour (in UTZ) to end of the day on the specified date")
parser.add_argument("output_dir", help="name of the directory that will contain the jsons with the changes")
#parser.add_argument("", help="")
args = parser.parse_args()

projectPolygon = createPoly(args.poly_file)
...
for i, way in enumerate(ways):
    ...
    if timestamp.hour >= int(args.min_hour_utz):
        ...
        if len(feature_nodes) == 0: # do not store a way that does not have any new nodes
            ...
            if not isInsidePolygon(center, projectPolygon):
                continue
